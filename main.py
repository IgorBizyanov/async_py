...
async def animate_spaceship(canvas, ship_states, max_row, max_col):
    ...
    for state in cycle(ship_states):
        ...
        row_speed_coef, col_speed_coef = update_speed(row_speed_coef,
                                                      col_speed_coef,
                                                      dy,
                                                      dx)
        ...
        for obst in obstacles:
            if obst.has_collision(obj_corner_row=row,
                ...
                coroutines.append(
                    show_gameover(canvas,
                                  mid_row-gameover_frame_size[0]//2,
                                  mid_column-gameover_frame_size[1]//2)
                )
                return
...
def draw(canvas):
    ...
    coroutines = [
        blink(canvas,
              ...
              randint(min_width, max_width),
              choice('+*.:')) for i in range(stars_num)
    ]
